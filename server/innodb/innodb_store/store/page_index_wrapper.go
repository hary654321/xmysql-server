...
type Index struct {
	...
	Tuple       tuple.TableRowTuple //表元祖信息，用于构建row
...
func NewPageIndex(pageNumber uint32) IPageWrapper {
...
func NewPageIndexWithSpaceId(spaceId uint32, pageNumber uint32) IPageWrapper {

	var slotRowData = NewSlotRows()
	return &Index{
		IndexPage:   pages.NewIndexPage(pageNumber, spaceId),
		SlotRowData: &slotRowData,
	}
}

func NewPageIndexWithTuple(spaceId uint32, pageNumber uint32, tuple tuple.TableRowTuple) IPageWrapper {
...
func NewPageIndexByLoadBytes(content []byte) IPageWrapper {
...
func NewPageIndexByLoadBytesWithTuple(content []byte, tuple tuple.TableRowTuple) IPageWrapper {
...
func (i *Index) GetPageHeader() pages.PageHeader {
...
func (i *Index) IsFull(row basic.Row) bool {
	...
	var rest = common.PAGE_SIZE - common.PAGE_FILE_HEADER_SIZE -
		common.PAGE_FILE_TRAILER_SIZE - common.PAGE_PAGE_HEADER_SIZE - common.PAGE_INFIMUMSUPERUM_SIZE - len(i.IndexPage.UserRecords) - len(i.IndexPage.PageDirectory)
...
func (i *Index) parseIndexBytes2SlotRows(content []byte, pageType string) {
	...
	for nextOffset != 0 {
		...
		if pageType == common.PAGE_INTERNAL {
			...
			case *SysTableInternalTuple:
				{
					currentRow = NewClusterSysIndexInternalRowWithContent(prepareContent, currentTuple)
				}
			case *ClusterLeafTuple:
				{
					currentRow = NewClusterLeafRowWithContent(prepareContent, currentTuple)
				}
			case *ClusterInternalTuple:
				{
					currentRow = NewClusterInternalRowWithContent(prepareContent, currentTuple)
				}
			case *SecondaryInternalTuple:
				{
					currentRow = NewSecondaryInternalRow(prepareContent, currentTuple)
				}
			case *SecondaryLeafTuple:
				{
					currentRow = NewSecondaryInternalRow(prepareContent, currentTuple)
				}
...
func (i *Index) FindByKey(targetKey basic.Value) (row basic.Row, found bool) {
	...
	index := sort.Search(len(fullList), func(i int) bool {
		cmp, _ := fullList[i].GetPrimaryKey().LessOrEqual(targetKey)
		return cmp.Raw().(bool)
	})
	//
	if index == 0 {
	...
	cmp, _ := targetKey.LessOrEqual(fullList[index-1].GetPrimaryKey())
	if index > 0 && (cmp.Raw().(bool)) {
		return fullList[index-1], true
	}
