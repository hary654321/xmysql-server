...
func (d *ddl) addTableIndex(t schemas.Table, indexInfo *model.IndexInfo, reorgInfo *reorgInfo, job *model.Job) error {
	cols := t.Cols()
	...
	for i := 0; i < workerCnt; i++ {
		...
		//	workers[i].index = schemas.NewIndexWithBuffer(t.Meta(), indexInfo)
	}
	for {
		startTime := time.Now()
		wg := sync.WaitGroup{}
...
func (w *worker) doBackfillIndexTask(t schemas.Table, colMap map[int64]*types.FieldType, wg *sync.WaitGroup) {
	defer wg.Done()
