...
func (b *planBuilder) buildAnalyzeTable(as *ast.AnalyzeTableStmt) Plan {
	...
	for _, tbl := range as.TableNames {
		...
		for _, idx := range idxInfo {
			p.IdxTasks = append(p.IdxTasks, AnalyzeIndexTask{TableInfo: tbl.TableInfo, IndexInfo: idx, PushDown: false})
		}
		if len(colInfo) > 0 || pkInfo != nil {
			p.ColTasks = append(p.ColTasks, AnalyzeColumnsTask{TableInfo: tbl.TableInfo, PKInfo: pkInfo, ColsInfo: colInfo, PushDown: false})
		}
	}
...
func (b *planBuilder) buildAnalyzeIndex(as *ast.AnalyzeTableStmt) Plan {
	...
	for _, idxName := range as.IndexNames {
		...
		p.IdxTasks = append(p.IdxTasks, AnalyzeIndexTask{TableInfo: tblInfo, IndexInfo: idx, PushDown: false})
	}
	p.SetSchema(&expression.Schema{})
	return p
}
...
func buildColumn(tableName, name string, tp byte, size int) *expression.Column {
	...
	return &expression.Column{
		...
		DBName:  model.NewCIStr(schemas.Name),
		RetType: fieldType,
	}
}
...
func (b *planBuilder) getDefaultValue(col *schemas.Column) (*expression.Constant, error) {
	value, err := schemas.GetColDefaultValue(b.ctx, col.ToInfo())
	if err != nil {
...
func (b *planBuilder) findDefaultValue(cols []*schemas.Column, name *ast.ColumnName) (*expression.Constant, error) {
...
func (b *planBuilder) resolveGeneratedColumns(columns []*schemas.Column, onDups map[string]struct{}, mockPlan LogicalPlan) (igc InsertGeneratedColumns) {
...
func (b *planBuilder) buildInsert(insert *ast.InsertStmt) Plan {
	...
	if !ok {
		b.err = schemas.ErrTableNotExists.GenByArgs()
		return nil
	}
	...
	columnByName := make(map[string]*schemas.Column, len(insertPlan.Table.Cols()))
...
func (b *planBuilder) buildLoadData(ld *ast.LoadDataStmt) Plan {
	...
	if !ok {
		...
		b.err = schemas.ErrTableNotExists.GenByArgs(db, tableInfo.Name.O)
		return nil
	}
...
func buildShowSchema(s *ast.ShowStmt) (schema *expression.Schema) {
	...
	case ast.ShowColumns:
		names = schemas.ColDescFieldNames(s.Full)
	case ast.ShowWarnings:
