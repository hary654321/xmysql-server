...
var EvalSubquery func(p PhysicalPlan, is schemas.InfoSchema, ctx context.Context) ([][]types.Datum, error)
...
func evalAstExpr(expr ast.ExprNode, ctx context.Context) (types.Datum, error) {
	...
	if ctx.GetSessionVars().TxnCtx.InfoSchema != nil {
		b.is = ctx.GetSessionVars().TxnCtx.InfoSchema.(schemas.InfoSchema)
	}
...
func (er *expressionRewriter) rewriteVariable(v *ast.VariableExpr) {
	...
	if !v.IsSystem {
		...
		f, err := expression.NewFunction(er.ctx,
			ast.GetVar,
			// TODO: Here is wrong, the sessionVars should storebytes a name -> Datum map. Will fix it later.
